...
<input type="button" name="next" class="next button" value="Next">
	...
	<div class="form">
		...
		<button type="button" class="btn btn-success btn-lg" data-toggle="modal" data-target="#myModal">Open Map</button>
        </div>
	...
	$.ajax({
		...
		if (shop === undefined || shop.length == 0) {
			...
			for(var i in shop){
					...
					if(shop[i].activation !== "Inactive" && shop[i].activation !== "Marked" && shop[i].activation !== "Resubmitted"){
					location.href = "Home.php";
...
setTimeout(function(){
swal("Reminder!","Leave untouched if there's nothing to change to the data","warning");
        ...
        function submitForm() {
            ...
            $.ajax({
              ...
              contentType: false   // tell jQuery not to set contentType
				...
				if (data == "error1"){
				setTimeout(function() {
                swal({title: "Sorry! File Error", text: "We only accept files with 5mb max file size and with file extensions of jpeg and png", icon: "error"})
				.then(function() {
					window.location = "registerShop.php";
				}); }, 1000);
				
				}else if (data == "error2") {
				setTimeout(function() {	
				swal({title: "Sorry!", text: "File Already Exist", icon: "error"})
				.then(function() {
					window.location = "registerShop.php";
				}); }, 1000);
				}else if (data == "error3"){
				setTimeout(function() {
				swal({title: "Sorry!", text: "There was a problem uploading your application", icon: "error"})
				.then(function() {
					window.location = "registerShop.php";
				}); }, 1000);
				...
				setTimeout(function() {
				swal({title: "Good Job!", text: "Your application form this needs to be checked first, Please wait for a while before your shop to be activated", icon: "success"})
				.then(function() {
					window.history.go(-1);
  ...
  <div class="modal fade" id="myModal" role="dialog" style="z-index:9999">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <button type="button" class="btn btn-success" id="myBtn" style="margin-right:5px;">Get current location</button>
          </div>
...
<div class="modal fade" id="myModal1" role="dialog" style="z-index:9500">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
  ...
  <table class="mytable table" id="tableinfo">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success" id="submitform" onclick="confirmSubmit()">Submit</button>
        </div>
      </div>
    </div>
  </div>
