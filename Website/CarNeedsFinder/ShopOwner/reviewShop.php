...
<div class="content-wrapper">
      ...
      <div class="well well-lg bg-myblue-color"><p style="font-size:30px; font-weight:bold; font-family:Arial, Helvetica, sans-serif;" id="pageHeader">User Reviews</p></div>
...
<table id="data" class="display responsive nowrap dataTable no-footer dtr-inline collapsed table table-bordered table-striped" style="width:100%; border-color:black;">
        <thead class="bg-darkblue-gradient">
            <tr>
                ...
                <th>Shop Name</th>
                <th>Shop Type</th>
                <th style="max-width:80px;">Review Count</th>
                <th></th>
            </tr>
        </thead>
	...
	$.ajax({
		...
		success: function(msg){
			if(msg == 1){
				...
				.then(function() {
					window.location = "activeShop.php";
				}); }, 1000);
				}else{
				setTimeout(function() {
...
fetchData();
	...
	$.ajax({
		type: "POST",
		url: "../php/fetchShopReviewCountbyShopOwner.php",
		data: "id="+id,
		...
		var shop = JSON.parse(data);
			for(var i in shop){
							...
							output += '<tr>' +
										...
										'<td style="max-width:180px;"><span style=" text-overflow: ellipsis; break-word: break-word ; display: block;  overflow: hidden"><strong>'+ shop[i].reviewcount +'</strong></span></td>' +
		...
		var shop = JSON.parse(data);
			for(var i in shop){
				$(function () {
					...
					$('#shopinfo').attr("href", 'shopExtras.php?cnf='+'\''+window.btoa(shop[i].idshop)+'\''+'')
...
<div class="modal fade" id="myModal" role="dialog" style="z-index:9500">
  ...
  <table class="mytable table" id="tableinfo">
        ...
        <div class="modal-footer">
          ...
          <a target="_blank" rel="noopener noreferrer" id="shopinfo"><button class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span>&nbsp;Add and Edit Information</button></a>
        </div>
      </div>
    </div>
  </div>
